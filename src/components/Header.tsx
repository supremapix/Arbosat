...
const LogoTree = () => (
  <motion.img 
    ...
    alt="Arbosat Logo" 
    className="w-12 h-12 min-w-12 min-h-12 object-contain filter drop-shadow-sm select-none"
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md border-b border-neutral-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <a
            href="https://wa.me/554199107517?text=Olá%20Arbosat,%20gostaria%20de%20solicitar%20um%20orçamento"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-teal hover:bg-brand-teal/90 text-white font-extrabold text-xs tracking-wider uppercase transition-all shadow-sm"
          >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-b-2 border-brand-teal/25 px-4 pt-4 pb-8 space-y-6 shadow-xl leading-relaxed">
          <div className="grid grid-cols-1 gap-1.5 pt-2">
            {[
              ...
              <button
                key={link.id}
                onClick={() => scrollToSection(link.id, link.name)}
                className="text-left py-4 px-5 rounded-xl hover:bg-neutral-50 text-neutral-900 hover:text-brand-teal font-extrabold text-base sm:text-lg border-l-4 border-neutral-200 hover:border-brand-lime transition-all flex items-center justify-between cursor-pointer"
              >
          ...
          <div className="border-t border-neutral-200 pt-5 px-1 flex flex-col gap-4">
            <span className="text-xs font-mono font-black text-neutral-500 uppercase tracking-widest text-center block">
              📞 CONTATO DIRECTO E DE ALTO CONTRASTE PARA IDOSOS
            </span>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-3 py-4.5 rounded-2xl bg-brand-teal hover:bg-brand-teal/95 text-white font-extrabold text-lg sm:text-xl shadow-md transition-all text-center"
            >
            ...
            <a
              ...
              className="w-full flex items-center justify-center gap-3 py-4.5 rounded-2xl bg-[#002f29] text-[#b4d423] border-2 border-brand-teal font-extrabold text-lg sm:text-xl shadow-md transition-all text-center"
            >
              <Phone className="w-5 h-5 fill-[#b4d423]" />
              <span>Ligar Grátis: (41) 9910-7517</span>
            </a>
          </div>
        </div>
      )}
